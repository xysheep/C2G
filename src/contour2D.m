function contour2D(x,y, outlier_percentage, color)
...
if exist('color','var')
    [~, h] = contour(XI,YI,Z,prctile(Y(ind:end),[0:15:60,70:10:90,93:3:99, 99:0.5:100]));
    h.LineColor = color;
else
    contour(XI,YI,Z,prctile(Y(ind:end),[0:15:60,70:10:90,93:3:99, 99:0.5:100]));
end
axis_tmp = axis;
...
if exist('color','var')
    plot(x(flag_dense_points==0),y(flag_dense_points==0),'.','color',color,'markersize',4);
else
    plot(x(flag_dense_points==0),y(flag_dense_points==0),'.','color',[0.3 0.3 0.3],'markersize',4);
end


if exist('min_value') && exist('max_value')

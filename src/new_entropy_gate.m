function [entropy,metric] = new_entropy_gate(labels,gatelabels)
...
for il = 1:length(all_l)
        ...
        if (all_l(il)==0)%If unlabeled cells are in some gates, this is still a overlap
...
%     entropy = 1 - I/sqrt(sum(px.*log(px))*sum(py.*log(py)));
% else
%     entropy = 1;
% end
